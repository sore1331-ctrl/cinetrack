...
test.describe('tracker data integrity', () => {
  ...
  test('tracked TV calendar uses TVMaze data for card highlights', () => {
    ...
    expect(app).toContain("(m.mediaType === 'tv' || m.mediaType === 'anime')");
  ...
  test('calendar model formats keys, dates, and airing-today signals', () => {
    ...
    const animeTodayRows = model.trackedRows({
      ...
      cache: {
        byId: {
          'anilist:777': { source: 'tvmaze', nextEpisode: { season: 1, episode: 10, airDate: '2026-06-03' } },
        },
      },
      todayStr: '2026-06-03',
      tvHorizonStr: '2026-06-17',
      keyFor: model.keyForEntry,
      infoUrlForEntry: entry => `/anime/${entry.externalId}`,
    });
  ...
  test('modal UI state is routed through the modal model', () => {
    ...
    expect(app).toContain('const modalSubmit = modalController.createSubmitController({');
    expect(app).toContain('const submission = modalSubmit.buildSubmission({');
  ...
  test('profile exposes local recovery snapshots', () => {
    ...
    expect(app).toContain('profileController.attachProfileInteractions');
    expect(profileView).toContain('function renderProfileHtml');
    expect(profileController).toContain('function attachProfileInteractions');
    expect(profileController).toContain('profileModel?.backupImpactLabel(compare)');
    expect(profileView).toContain('data-restore-backup');
    expect(profileController).toContain('restoreLibraryFromBackup?.(backup.movies)');
